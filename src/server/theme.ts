...
// return only theme cover, theme name and theme title from theme collection
router.get('/cover', (req: Request, res: Response) => {
  themes.find({}, {"cover": 1, "title": 1, "name": 1}).then((result) => {
    res.send(result);
  });
});
